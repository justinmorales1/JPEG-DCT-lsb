    ...
    if (compression == BI_RGB)
    {
      ...
      else
      {
        for (unsigned int ii = 0 ; ii < - height ; ++ ii)
        {
          ...
          strm.read ((char *)&image [ii][0], rowwidth) ;
          if (strm.gcount () != rowwidth)
            throw EBmpFileReadError () ;
  ...
  else
  {
    ...
    else
    {
      for (unsigned int yy = 0 ; yy < -height ; ++ yy)
      {
        ...
        for (unsigned int xx = 0 ; xx < 3 * width ; xx += 3)
        {
          strm.read ((char *)&image[ii][xx + BitmapImage::BlueOffset], 1) ;
          strm.read ((char *)&image[ii][xx + BitmapImage::GreenOffset], 1) ;
          strm.read ((char *)&image[ii][xx + BitmapImage::RedOffset], 1) ;
        }
        static char pad [4] ;
        strm.read (pad, padsize) ;
      }
